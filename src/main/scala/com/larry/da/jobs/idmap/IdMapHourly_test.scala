...
object IdMapHourly_test {
  var sc:SparkContext = _

  var parallelism = 30 // sc.defaultParallelism
  var grapParallelism = 10 // sc.defaultParallelism
  ...
  def computeHourList(sparkContex:SparkContext,para:Int,hourList:ArrayBuffer[String])={
    ...
    val rdd = sc.union(
      Config.getDataSource().flatMap(d=> fileHours.map(h => d.replace("{YYYY-MM-DD-HH}",h)))
        .map(s=>
          sc.textFile(s).map(x=>{
            val Array(time,id1,id1Type,id2,id2Type) = x.split("\t",-1).map(_.trim)
